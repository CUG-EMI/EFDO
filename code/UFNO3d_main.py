...
usage: python UFNO3d_main.py UFNO3d_config
Original network ref: https://github.com/gegewen/ufno
"""
...
def get_batch_data(TRAIN_PATH, VAL_PATH, TEST_PATH,   
    ...
    key_map0 = ['rhoxy','rhoyx']  # we only use rhoxy and rhoyx
    key_map = key_map0[:n_out] # number of output channels is 2 (rhoxy, rhoyx)
    t_read0 = default_timer()
    
    # get training data
    reader = MatReader(TRAIN_PATH) # class in utilities
...
def main(item):
    ...
    '''
    t0 = default_timer()
    # item name in config_UFNO.yml file
    ...
    # setup UFNO-3d network structure
