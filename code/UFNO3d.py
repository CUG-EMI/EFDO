...
class UFNO3d(nn.Module):
    def __init__(self, modes1, modes2, modes3, width, nout, layer_sizes, init_func, act_func="gelu"):
    ...
    def forward(self, x, freq):
        ...
        nFreq = freq.shape[0]
        x1 = torch.zeros(batchsize, size_x, size_y, nFreq, 1).to(x.device)
        for i in range(nFreq):
            x1[..., i, :] = x * freq[i]
        x = F.pad(F.pad(x1, (0,0,0,8,0,8), "replicate"), (0,0,0,0,0,0,0,8), 'constant', 0)
        x = self.conv1(x)
        x = self.fc1(x)
        x = self.activation(x)
        x = self.fc2(x)
        x = x[..., :-8,:-8,:-8, :]
        x = x.permute(0, 3, 1, 2, 4) # batchsize, nFreq, x, y, nout
        x = x.contiguous().view(batchsize, nFreq, -1, self.nout)
        x = x.permute(0, 1, 3, 2) # batchsize, nFreq, nout, x*y
        for i in range(len(self.linears)-1):
            x = self.activation(self.linears[i](x))
        x = self.linears[-1](x)
        x = x.permute(0, 1, 3, 2) # batchsize, nFreq, nSite, nout
